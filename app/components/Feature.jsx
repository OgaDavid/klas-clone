...
export const FeatureCard = ({data}) => {
    return (
        <div className="bg-[#f8f9f9] flex flex-col gap-[50px] py-[40px] px-[30px] runded-[3px]">
            <img className="object-contain aspect-[2/1]" src={data.img} alt={data.heading} />
            <div>
                <h3 className="text-lg leading-4 mb-[10px] font-bold">{data.heading}</h3>
                <p className="text-sm leading-[18px] font-normal text-[rgba(0,0,0,.5)]">{data.paragraph}</p>
            </div>
        </div>
    )
}
